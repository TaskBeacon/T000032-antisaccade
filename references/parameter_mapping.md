...
| timing.fixation | `timing.fixation_duration` | Human `[0.8,1.2]`; QA `[0.3,0.4]` | W1989816441 | Fixation epoch precedes preparatory rule cue and target onset. | inferred | Sampled per trial by deterministic helper in `src/utils.py`. |
| timing.rule_cue | `timing.cue_duration` | Human `[0.4,0.6]`; QA `[0.2,0.3]` | W2104663113 | Preparatory-set cue duration supports pro/anti rule preparation. | inferred | Rendered as colored rule text stimulus and sampled in `src/utils.py`. |
| timing.gap | `timing.gap_duration` | Human `[0.15,0.25]`; QA `[0.08,0.12]` | W1875187353 | Gap between cue and target is commonly used in antisaccade control paradigms. | inferred | Triggered by `gap_onset` and sampled in `src/utils.py`. |
| timing.response_deadline | `timing.response_deadline` | Human `1.0s`; QA `0.8s` | W2163144416 | Bounded response window supports hit/error/timeout classification. | inferred | Timeout handling is implemented directly in `src/run_trial.py`. |
| timing.iti | `timing.iti_duration` | Human `0.6s`; QA `0.2s` | W1989816441 | ITI separates sequential oculomotor control events. | inferred | Sampled per trial by deterministic helper in `src/utils.py`. |
